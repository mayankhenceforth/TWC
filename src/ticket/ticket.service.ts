...
export class TicketService {
  ...
  if (game.isTicketGenrated == true) throw new BadRequestException("Ticket already created");
  ...
  switch (createTicketDto.title) {
    ...
    case 'super_heros': {
      ...
      const allHeroes = response.data.map((hero: any) => hero.name);
      titles = allHeroes.sort(() => 0.5 - Math.random()).slice(0, numberOfTickets);
      break;
    }
    default: {
      const response = await axios.get(`https://api.mockster.dev/api/v1/animals?Count=${numberOfTickets}`);

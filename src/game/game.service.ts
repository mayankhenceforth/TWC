...
import { GameParticipation, GameParticipationDocument } from "src/schema/gameParticipation.schema";
import { CreateGameDto } from "./dto/create.game.dto";
import { UpdateGameDto } from "./dto/update.game.dto";
import { GameStatus } from "src/enums/game.enum";
...
export class GameService {
  ...
  async create(createGameDto: CreateGameDto, userId: Types.ObjectId) {
    const existingGame = await this.gameModel.find({ title: createGameDto.title });
    if (existingGame.length > 0) throw new BadRequestException("This game title is already taken");

    if (new Date(createGameDto.startTime) <= new Date()) throw new BadRequestException('Start time must be in the future');

    if (createGameDto.numberOfPlayers < 1) throw new BadRequestException('Max participants must be at least 1');
 ...
 async update(updateGameDto: UpdateGameDto, userId: Types.ObjectId, gameId: Types.ObjectId) {
    const game = await this.gameModel.findById(gameId);
    if (!game) throw new BadRequestException("Game not found");

    if (!game.createdBy.equals(userId)) 
        throw new BadRequestException("You are not allowed to update this game");

    if (game.status !== GameStatus.UPCOMING) 
        throw new BadRequestException("Game cannot be updated at this stage");
